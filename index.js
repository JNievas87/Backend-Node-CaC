...
app.get('/generos/getall', async (req, res) => {
...
app.get('/generos/get/:id', async (req, res)=>{
    const id= req.params.id;
    const sql= 'SELECT * FROM generos ORDER BY generos.id asc';
    try {
        const connection = await pool.getConnection()
...
app.post('/generos/insert', async (req, res) =>{
...
//Actualizar genero
app.put('/generos/update/:Id', async (req, res)=>{
    const id = req.params.Id;
    const genero = req.body;
...
//Borrar director
app.delete('/generos/delete/:Id', async (req, res)=>{
    const id = req.params.Id;
